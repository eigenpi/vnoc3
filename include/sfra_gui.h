...
class GUI_GRAPHICS {
 ...
 private:
	...
	FILE *_ps_file;
	int _picture_count; // keeps track of saved .ps pictures;
	...
	// this records whihc side, x or y, is the larger one for a given
	// floorplan; this is needed because floorplans may have aspect ratio
	// diffrent from 1;
	double _bigger_side;
 ...
 public:
	TOPOLOGY *_topology;
	VNOC *_vnoc;
	// next ones are related to the floorplanning and router assignmemnt;
	ONE_OF_THE_BEST_FLOOR_PLANS *_best_fp1;
	ONE_OF_THE_BEST_FLOOR_PLANS *_best_fp2; // used only in 3D sim_mode;
	ROUTER_ASSIGNMENT *_router_assignment;
	ROUTERS_DISTRIBUTION *_router_distribution;

	// scale is used to define the size of text and routers as different
	// floorplanings may have different sizes making the routers too tiny
	// or too big;
	float _scale;
	// testcase name; will be included in any snapshot saved;
	char _testcase_name[256];

 public:
	GUI_GRAPHICS( TOPOLOGY *topology, VNOC *vnoc,
		char *testcase_name,
		ONE_OF_THE_BEST_FLOOR_PLANS *best_fp1,
		ONE_OF_THE_BEST_FLOOR_PLANS *best_fp2, // default we run 2.5D or 2D;
