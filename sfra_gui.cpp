	...
	PICTURE_TYPE pic_on_screen_val) 
{
	...
	if ( _pic_on_screen != pic_on_screen_val) {
		if ( pic_on_screen_val == ROUTERS && _pic_on_screen == NO_PICTURE) {
			create_button( (char *) "Window", (char *) "Toggle Links", TOGGLE_LINKS);
			create_button( (char *) "Toggle Links", (char *) "Toggle Occ", TOGGLE_OCC);
			create_button( (char *) "Toggle Occ", (char *) "Skip steps", SKIP_STEPS);
		}
	}
	...
	// here, automatically save a snapshot of the current floorplaning;
	char fname[281];
	sprintf(fname, "results/%s-%.0fx%.0f-attempt%d.ps", &_testcase_name,
		_bigger_side, _bigger_side, _best_fp1->attempt_n);
	//sprintf(fname, "results/attempt%d.ps", _best_fp1->attempt_n);
	if (init_postscript(fname)) {
		drawscreen();
		close_postscript();
	} else {
		printf("\n\nERROR: could not open PS file for floorplaning snapshot.\n\n");
	}
...
void GUI_GRAPHICS::redraw_screen (void) 
{
	...
	if ( _pic_on_screen == ROUTERS) {
		...
		for ( long r_i = 0; r_i < routers_count; r_i++) {
			...
			// calculate on the fly how many flits occupy currently this router;
		...
		for ( long r_i = 0; r_i < routers_count; r_i++) {
			...
			// sequence of colors from black to white;
		...
		// (d) do the drawing with the updated occupancies;
		draw_routers();
	}
}
...
void GUI_GRAPHICS::toggle_occ(void) 
{
	// Enables/disables drawing of routing resources. See graphics.c for details
	// of how buttons work.

	_show_congestion = !_show_congestion;
	...
	float channel_width_val)
{
	...
	for ( i = 0; i < routers_count; i++) {
		...
		if ( _vnoc->_routers[i]._assigned_core != -1) {
			_router_has_PE[i] = true;
		} else {
			_router_has_PE[i] = false;
		}
	}

	// determine the region that is displayed/seen;
	_bigger_side = fmax( _best_fp1->Width, _best_fp1->Width);
	// if sim_mode is 3D, then we also have subfloorplan 2 and have to 
	// recompute and overwrite _bigger_side;
	if ( _vnoc->sfra()->sim_mode() == SIMULATED_ARCH_3D) {
		double max_width  = fmax( _best_fp1->Width, _best_fp2->Width);
		double max_height = fmax( _best_fp1->Height, _best_fp2->Height);
		_bigger_side = fmax( max_width, max_height);
	}

	init_world(-60*_scale, _bigger_side + 60*_scale,
		_bigger_side + 60*_scale, -60*_scale);
}
...
void GUI_GRAPHICS::draw_routers (void) 
{
	...
	long nx = _router_distribution->nx();
	long ny = _router_distribution->ny();
	CORNER_NAME corner;
	pair <int, int> mesh_pos;

	setlinewidth(0);



	// () version for 2.5D and 2D;
	if ( _vnoc->sfra()->sim_mode() == SIMULATED_ARCH_25D ||
		_vnoc->sfra()->sim_mode() == SIMULATED_ARCH_2D) {


		// draw the cores;	
		for (int n = 0; n < _best_fp1->modules_N; n++) {
			x1 = _best_fp1->modules_info[n].x;
			x2 = _best_fp1->modules_info[n].rx;
			y1 = _best_fp1->modules_info[n].y;
			y2 = _best_fp1->modules_info[n].ry;
			...
			setcolor ( BLACK);
			sprintf ( buf, "ID %d", n);
			drawtext ( (x1 + x2)/2., (y1 + y2)/2., buf, 100*_scale);
		}

		// draw the assigned routers and the wires between them and the cores;
		for ( int n = 0; n < _best_fp1->modules_N; n++) {
			mesh_pos = _router_assignment->core_id_to_router_xy(n);
			xr = mesh_pos.first * _router_distribution->link_length() +
				_router_distribution->sx();
			yr = mesh_pos.second * _router_distribution->link_length() +
				_router_distribution->sy();

			if (_show_links) {
				corner = _router_assignment->corner_used_by_id(n);
				setlinestyle(SOLID);
				setcolor (BLUE);
				switch (corner) {
				case SW:
					drawline(xr, yr, _best_fp1->modules_info[n].x,
							 _best_fp1->modules_info[n].y);
					break;
				case SE:
					drawline(xr, yr, _best_fp1->modules_info[n].rx,
							 _best_fp1->modules_info[n].y);
					break;
				case NW:
					drawline(xr, yr, _best_fp1->modules_info[n].x,
							 _best_fp1->modules_info[n].ry);
					break;
				case NE:
					drawline(xr, yr, _best_fp1->modules_info[n].rx,
							 _best_fp1->modules_info[n].ry);
			...
			long this_router_id = mesh_pos.first * nx + mesh_pos.second;
			setlinestyle(SOLID);
			setcolor ( _router_color[this_router_id]);
			fillrect ( xr-20*_scale, yr-20*_scale, xr+20*_scale, yr+20*_scale);
			setcolor (BLUE);
			drawrect ( xr-20*_scale, yr-20*_scale, xr+20*_scale, yr+20*_scale);
			sprintf ( buf, "%d", n);
			drawtext ( xr, yr, buf, 50*_scale);
			if ( _show_congestion) {
				setcolor (RED);
				sprintf ( buf, "Occ: %.0f", _router_occupancy[this_router_id]);
				drawtext ( xr+20*_scale, yr+40*_scale, buf, 200*_scale);
			}
		}
	}
	...
	// () version for 3D;
	else if ( _vnoc->sfra()->sim_mode() == SIMULATED_ARCH_3D) {
		...
		for (int n = 0; n < _best_fp1->modules_N; n++) {
			...
			setcolor ( BLACK);
			drawrect ( x1,y1,x2,y2);
			int top_n = _vnoc->sfra()->magic_subid1_to_topid( n);
			setcolor ( BLACK);
			sprintf ( buf, "ID %d", top_n);
			drawtext ( (x1 + x2)/2., (y1 + y2)/2., buf, 100*_scale);
		}
		for (int n = 0; n < _best_fp2->modules_N; n++) {
			x1 = _best_fp2->modules_info[n].x;
			x2 = _best_fp2->modules_info[n].rx;
			y1 = _best_fp2->modules_info[n].y;
			y2 = _best_fp2->modules_info[n].ry;
			...
			int top_n = _vnoc->sfra()->magic_subid2_to_topid( n);
			setcolor ( BLACK);
			sprintf ( buf, "ID %d", top_n);
			drawtext ( (x1 + x2)/2., (y1 + y2)/2., buf, 100*_scale);
		}

		// links for modules of subfloorplan 1;
		for ( int n = 0; n < _best_fp1->modules_N; n++) {
			// get the top-level module id;
			int top_n = _vnoc->sfra()->magic_subid1_to_topid( n);
			
			mesh_pos = _router_assignment->core_id_to_router_xy( top_n);
			xr = mesh_pos.first * _router_distribution->link_length() +
				_router_distribution->sx();
			yr = mesh_pos.second * _router_distribution->link_length() +
				_router_distribution->sy();
			...
			if (_show_links) {
				corner = _router_assignment->corner_used_by_id( top_n);
				setlinestyle(SOLID);
				setcolor (BLUE);
				switch (corner) {
				case SW:
		...
		// links for modules of subfloorplan 2;
		for ( int n = 0; n < _best_fp2->modules_N; n++) {
			// get the top-level module id;
			int top_n = _vnoc->sfra()->magic_subid2_to_topid( n);
			...
			if (_show_links) {
				...
				switch (corner) {
				case SW:
					drawline(xr, yr, _best_fp2->modules_info[n].x,
							 _best_fp2->modules_info[n].y);
					break;
				case SE:
					drawline(xr, yr, _best_fp2->modules_info[n].rx,
							 _best_fp2->modules_info[n].y);
					break;
				case NW:
					drawline(xr, yr, _best_fp2->modules_info[n].x,
							 _best_fp2->modules_info[n].ry);
					break;
				case NE:
					drawline(xr, yr, _best_fp2->modules_info[n].rx,
							 _best_fp2->modules_info[n].ry);
	...
	// () draw the unassigned routers;
	for ( int y = 0; y < ny; y++) {
		for ( int x = 0; x < nx; x++) {
			long this_router_id = x * nx + y;
			if (_router_assignment->assigned(x,y)==-1) {
				xr = x * _router_distribution->link_length() + _router_distribution->sx();
				yr = y * _router_distribution->link_length() + _router_distribution->sy();
				setlinestyle(SOLID);
				setcolor (_router_color[this_router_id]);
				fillrect ( xr-20*_scale, yr-20*_scale, xr+20*_scale, yr+20*_scale);
				setcolor (RED);
...
{
	// Moves view 1/4 screen left.
	float xstep;
	xstep = (_xright - _xleft)/4.;
	_xleft -= xstep;
	_xright -= xstep; 
	update_transform();			
	drawscreen();
...
void GUI_GRAPHICS::postscript () 
{
	...
	sprintf(fname, "results/pic%d.ps", _picture_count);
...
GUI_GRAPHICS::GUI_GRAPHICS( TOPOLOGY *topology, VNOC *vnoc,
	char *testcase_name,
	ONE_OF_THE_BEST_FLOOR_PLANS *best_fp1,
	ONE_OF_THE_BEST_FLOOR_PLANS *best_fp2,
	ROUTER_ASSIGNMENT *router_assignment,
	ROUTERS_DISTRIBUTION *router_distribution)
{
	strcpy( _testcase_name, testcase_name);
	...
	_router_distribution = router_distribution;
	...
	// snapshots counter; these snapshots are saved as .ps pictures;
	_picture_count = 1;
	// this is an empirical value (ami33 was taken as reference for _scale = 1);
	_scale = _router_distribution->link_length() / 250;
}
...
bool GUI_GRAPHICS::build()
{
	...
	// window title shows the floorplaning attempt being depicted
	// set up the display graphics;
	sprintf(buf, "Attempt #%d - Simultaneous Floorplaning and Router Assignment",
		_best_fp1->attempt_n);
	init_graphics(buf);
...
{
	const static char *ps_cnames[COLORS_NUMBER] = {"white", "black", 
...
{ 
	...
	const static char *ps_text[2] = {"linesolid", "linedashed"};
...
void GUI_GRAPHICS::init_graphics( char *window_name) 
{
	...
	const char *cnames[COLORS_NUMBER] = { "white", "black", "grey55", "grey75", "blue",
	...
	for (i = 0; i <= MAX_FONT_SIZE; i++)
		...
		_top_width = 3 * _display_width / 4;
		_top_height = 5 * _display_height / 6;
	...
	while (1) {
		...
		case ButtonPress:
			if ( report.xbutton.window == _toplevel) {
				...
				drawbut( bnum);
				if ( _button[bnum].fcn == PROCEED ||
					 _button[bnum].fcn == SKIP_STEPS ) { // proceed
					turn_on_off(OFF);
					flushinput();
					return; 
...
void GUI_GRAPHICS::execute_action_associated_with_button_function( BUTTON_FUNCTION fcn)
{
	...
	case TOGGLE_OCC:
		...
		break;
	case SKIP_STEPS:
		_wait_for_user_input_automode=2;
		proceed();
		break;
	default:
		assert(0);
