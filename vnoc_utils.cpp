...
// tricks to accomodate compilation on Windowz;
...
FILE *my_fopen( const char *fname, char *mode, int prompt) 
{
	...
	while (1) {
		...
		if ( (fp = fopen(fname, mode)) != NULL)
			break;
		printf("Error:\tUnable to open file %s for %s access.\n", fname, mode);
		if ( !prompt) exit(1);
...
{
	// changed version for Windowz compilation issues;
	double val = random() * 1.0 / (RAND_MAX + 1.0);
